...
视频链接
  ...
  → 人工轨优先 → 完整读取整份时间轴 cues
  → 删除口语噪声，保留全部实质信息，重组为自然文章
  → 完整翻译为用户语言（需要时）
  → 先交付完整文章，再做摘要、问答等后续操作
  → 访问失败：回退到用户本地浏览器
...
| 确认后下载并返回完整来源字幕/cues | ✅ Chromium mock 实测 |
| 来源字幕 cue 读取完整性校验 | ✅ 纯 Python + Chromium mock 实测 |
...
这条路径把完整来源字幕、cues 和读取覆盖报告交给 Agent；返回值带 `requiresEditorialPass`，明确它还不是成品。Agent 删除填充词、失败起句和无意义重复，保留观点、事实、例子、数字、条件和结论，重组并翻译成完整文章后才交付。自动跳广告只对该 video id 生效；YouTube SPA 切换到其他视频后不再点击。
